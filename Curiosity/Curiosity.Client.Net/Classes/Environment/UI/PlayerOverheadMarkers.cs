    ...
    static class PlayerOverheadMarkers
    {
        ...
        static internal async Task DrawCurrentlyTalking()
        {
            try
            {
                ...
                {
                    UI.DrawText($"Currently Talking", new Vector2(0.5f, 0.02f), Color.FromArgb(255, 106, 76, 147), 0.3f, 0, Alignment.Center);
                    UI.DrawText(currentlyTalking, new Vector2(0.5f, 0.043f), Color.FromArgb(255, 255, 255, 255), 0.26f, 0, Alignment.Center);
                }
            }
            catch (Exception ex)
            {
        ...
        static async Task DrawMarker(CitizenFX.Core.Player player, int distanceRank)
        {
            ...
            if ((Function.Call<bool>(Hash.NETWORK_IS_PLAYER_TALKING, player) || (player == Game.Player && ControlHelper.IsControlPressed(Control.PushToTalk, true, ControlModifier.Any))))
            ...
            if (ControlHelper.IsControlPressed(Control.CharacterWheel, true, ControlModifier.Any) && distanceRank.IsBetween(0, 10))
            {
                ...
                if (scaleformHandles[player.Handle].IsLoaded)
                {
                    ...
                    scaleformHandles[player.Handle].Render3D(player.Character.Position + OverheadIdTextOffset, Function.Call<Vector3>(Hash.GET_GAMEPLAY_CAM_ROT, 2), 1f * Vector3.One);
                }
            }
            else
            {
