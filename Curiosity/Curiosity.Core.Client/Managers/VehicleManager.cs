    ...
    public class VehicleManager : Manager<VehicleManager>
    {
        ...
        internal async Task<dynamic> CreateVehicle(int characterVehicleId, string hash)
        {
            try
            {
                ...
                {
                    Vector5 position = await VehicleGenerationPositions.GetNearestUnoccupiedParkingSpot();
                    if (!position.Vector3.IsZero && Game.PlayerPed.IsInRangeOf(position.Vector3, 50f))
                    {
                ...
                if (Cache.PersonalPlane is not null && vehicleItem.SpawnTypeId == SpawnType.Plane)
