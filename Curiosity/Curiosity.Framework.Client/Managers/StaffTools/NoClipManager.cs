    ...
    public class NoClipManager : Manager<NoClipManager>
    {
        ...
        private async Task OnNoClipControlTick()
        {
            try
            {
                ...
                if (ControlHelper.IsControlJustPressed(Control.SaveReplayClip))
                {
                    IsEnabled = !IsEnabled;
                    SetPlayerInvisibleLocally(Game.Player.Handle, IsEnabled);
                }
                ...
                if (!IsEnabled)
                {
                    if (CurrentCamera != null)
                    {
                        ...
                        float groundZ = pos.Z;
                        if (GetGroundZFor_3dCoord_2(pos.X, pos.Y, pos.Z, ref groundZ, false))
                ...
                {
                    if (droneProp.Exists())
                        SetEntityLocallyInvisible(droneProp.Handle);
                }
