...
{
    internal class MugshotBoardAttachment
    {
        ...
        public async Task Attach(Ped ped, User user, string topLine = "", string subTopLine = "", string mainLine = "", int importedColor = 0, int rankNumber = 1, int rankColor = 0)
        {
            ...
            API.AttachEntityToEntity(board, ped.Handle, API.GetPedBoneIndex(API.GetPlayerPed(-1), 58868),
                0.12f, 0.24f,
                0.0f, 5.0f, 0.0f, 70.0f, true, true, false, false, 2, true);
            API.AttachEntityToEntity(text, ped.Handle, API.GetPedBoneIndex(API.GetPlayerPed(-1), 58868),
                0.12f, 0.24f,
                0.0f, 5.0f, 0.0f, 70.0f, true, true, false, false, 2, true);
            ...
            while (API.HasScaleformMovieLoaded(movie) && IsAttached)
            {
                ...
                API.PushScaleformMovieFunctionParameterString(topLine); // Header String
                API.PushScaleformMovieFunctionParameterString(mainLine); // Number String
                API.PushScaleformMovieFunctionParameterString($"LOS SANTOS POLICE DEPT"); // Footer String
                API.PushScaleformMovieFunctionParameterString(subTopLine); // Imported String
                API.PushScaleformMovieFunctionParameterInt(importedColor); // Imported Color
                API.PushScaleformMovieFunctionParameterInt(rankNumber); // Rank Number
                API.PushScaleformMovieFunctionParameterInt(rankColor); // Rank Color
