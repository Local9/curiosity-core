...
namespace Curiosity.Systems.Client
{
    public static class CommonFunctions
    {
        ...
        {
            return API.N_0x921ce12c489c4c41(handle);
        }

        public static float GetPointingPitch()
        {
            float pitch = API.GetGameplayCamRelativePitch();
            if (pitch < -70f)
            {
                pitch = -70f;
            }
        ...
        {
            float heading = API.GetGameplayCamRelativeHeading();
            if (heading < -180f)
            {
                heading = -180f;
            }
        ...
        {
            bool hit = false;
            float rawHeading = API.GetGameplayCamRelativeHeading() / 90f;
            ...
            Vector3 pos1 = API.GetOffsetFromEntityInWorldCoords(Game.PlayerPed.Handle, pos0.X, pos0.Y, pos0.Z);
            int handle = API.StartShapeTestCapsule(pos1.X, pos1.Y, (pos1.Z - 0.2f), pos1.X, pos1.Y, (pos1.Z + 0.2f), 0.4f, 95, Game.PlayerPed.Handle, 7);
