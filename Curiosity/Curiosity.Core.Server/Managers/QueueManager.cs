    ...
    public class QueueManager : Manager<QueueManager>
    {
        ...
        private async void OnConnect([FromSource] Player player, string name, CallbackDelegate denyWithReason, dynamic deferrals)
        {
            try
            {
                ...
                deferrals.update("Checking users identities.");
                ...
                await BaseScript.Delay(500);
                deferrals.update("Checking users Discord information.");
                await BaseScript.Delay(500);
                ...
                if (isActive && PluginManager.IsLive)
                {
                    ...
                    CuriosityUser curiosityUser = PluginManager.ActiveUsers.Select(x => x.Value).Where(x => x.DiscordId == discordId).FirstOrDefault();

                    if (curiosityUser!= null)
                    {
                        RemoveFrom(curiosityUser.License, true, true, true, true, true, true);

                        Player matchingPlayer = PluginManager.PlayersList.Where(x => ulong.Parse(x.Identifiers["discord"]) == curiosityUser.DiscordId).FirstOrDefault();
                ...
                if (string.IsNullOrEmpty(rulesTemplate))
                {
                    Logger.Error($"FAILED TO LOAD '{GetCurrentResourceName()}/data/cards/join-server.json'");
                    deferrals.done($"Error when trying to joing the server, please ask the server owners to check the server log.");
                }

                await BaseScript.Delay(500);
                deferrals.update("Requesting server rules...");

                deferrals.presentCard(rulesTemplate, new Action<dynamic, string>(async (data, rawData) =>
                {
                    if (data.acceptRules != "true")
                    {
                        deferrals.done("You must accept the servers rules to connect.");
                        denyWithReason.Invoke("You must accept the servers rules to connect.");
                        return;
                    }
                    ...
                    deferrals.update("Checking server maintenance...");
                    ...
                    deferrals.update("Checking if server is in Supporter Only...");
