        ...
        public static async Task<GlobalEntity.User> GetUserWithCharacterAsync(string license, ulong discordId, Player player)
        {
            try
            {
                ...
                using (var result = mySql.QueryResult(selectQuery, myParams))
                {
                    ...
                    if (keyValuePairs.Count == 0)
                    {
                        Classes.DiscordWrapper.SendDiscordSimpleMessage(Enums.Discord.WebhookChannel.ServerErrors, "User Creation Exception", "GetUserWithCharacterAsync -> User Failed to Create", $"License: {license}\nServerID: {user.ServerId}");
            ...
            catch (Exception ex)
            {
                ...
                Classes.DiscordWrapper.SendDiscordSimpleMessage(Enums.Discord.WebhookChannel.ServerErrors, "User Creation Exception", $"GetUserWithCharacterAsync -> {ex}", $"\n\nLicense: {license}\nPlayer: {player.Name}");
