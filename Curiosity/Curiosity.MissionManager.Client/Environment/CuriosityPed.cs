...
namespace Curiosity.MissionManager.Client.Environment
{
    public class CuriosityPed
    {
        ...
        public void Init()
        {
            Instance.AttachTickHandler(OnTick);
        }

        public void Stop()
        {
            Instance.DetachTickHandler(OnTick);
        }

        private async Task OnTick()
        {
            if (Instance?.Local?.Entity != null)
            {
        ...
        private async void Load()
        {
            if (Instance?.Local?.Entity != null)
            {
                ...
                if (pedInArea == null)
                {
                    ...
                    Decorators.Set(_ped.Handle, Decorators.PLAYER_OWNER, Game.PlayerPed.Handle);
                }
                else
                {
                    _ped = pedInArea;
        ...
        private void Dispose()
        {
            if (Instance?.Local?.Entity != null)
            {
                if (_ped != null)
                {
                    ...
                    if (_ped.Exists())
                    {
                        int owner = Decorators.GetInteger(_ped.Handle, Decorators.PLAYER_OWNER);
