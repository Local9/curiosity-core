...
namespace Curiosity.Menus.Client.net.Classes.Menus.MissionCreator.Classes
{
    class NoClipController
    {
        ...
        static public float Speed { get; set; } = 1f;

        static public Camera CurrentCamera { get; set; }

        static public void EnterNoClip()
        {
        ...
        static public void ExitNoClip()
        {
            client.DeregisterTickHandler(OnTick);
            client.DeregisterTickHandler(CheckInputRotation);
        ...
        static private async Task OnTick()
        {
            try
            {
                ...
                if (Shared.Client.net.Helper.ControlHelper.IsControlJustPressed(Control.SaveReplayClip, true) && Player.PlayerInformation.IsDeveloper())
                {
                    IsEnabled = !IsEnabled;
                }
        ...
        static private async Task CheckInputRotation()
        {
            try
            {
                if (CurrentCamera == null)
