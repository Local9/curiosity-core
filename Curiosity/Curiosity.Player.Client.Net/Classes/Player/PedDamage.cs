    ...
    static class PedDamage
    {
        ...
        static public void CheckDamageLocation(Ped p)
        {
            try
            {
                ...
                {
                    BaseScript.TriggerEvent("curiosity:Client:Chat:Message", "", "#AABB88", $"You see {String.Join(",", result)}.");
                }
                else
                {
                    BaseScript.TriggerEvent("curiosity:Client:Chat:Message", "", "#AABB88", $"There is no damage that is clearly visible to you at the moment.");
                }
            }
            catch (Exception ex)
            {
        ...
        static public void CheckWeaponSources(Ped p)
        {
            try
            {
                ...
                {
                    BaseScript.TriggerEvent("curiosity:Client:Chat:Message", "", "#AABB88", $"You see damage that was probably caused by weapons such as: {String.Join(",", weaponDamageTypes.Select(w => w.ToString().AddSpacesToCamelCase()))}.");
                }
                else
                {
                    BaseScript.TriggerEvent("curiosity:Client:Chat:Message", "", "#AABB88", $"There is no damage that is clearly visible to you at the moment.");
