...
namespace Curiosity.System.Client
{
    public class CuriosityPlugin : BaseScript
    {
        ...
        private async Task OnTick()
        {
            ...
            var position = GetEntityCoords(PlayerPedId(), false);

            ClearAreaOfCops(position.X, position.Y, position.Z, 150f, 0);
        ...
        private async Task SaveTask()
        {
            if (Local?.Character != null)
            {
                ...
                Local.Character.Health = GetEntityHealth(Cache.Entity.Id);
                Local.Character.Shield = GetPedArmour(Cache.Entity.Id);
